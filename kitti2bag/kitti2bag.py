...
def save_camera_data(bag, kitti_type, kitti, util, bridge, camera, camera_frame_id, topic, initial_time):
    ...
    bar = progressbar.ProgressBar(max_value=len(image_filenames), 
            widgets=[progressbar.Percentage(), ' (', progressbar.Counter(), ' of ',
                     str(len(image_filenames)), ') ', progressbar.Bar('#'), ' ',
...
def save_velo_data(bag, kitti_type, kitti, velo_frame_id, topic, initial_time):
    ...
    bar = progressbar.ProgressBar(max_value=len(velo_filenames), 
            widgets=[progressbar.Percentage(), ' (', progressbar.Counter(), ' of ',
                     str(len(velo_filenames)), ') ', progressbar.Bar('#'), ' ',
                     progressbar.Timer(), ' ', progressbar.AdaptiveETA()])
    for dt, filename in bar(iterable):
        if dt is None:
            continue
